...
<div id="page" class="site">
	...
	<header class="fixed w-full z-50 left-0 transition-all duration-500 ease-in-out py-9 header">
    ...
    <div class="w-1/4 lg:w-5/12 text-white text-left lg:text-right lg:order-3">
        <a href="#" class="ctaButton">Apply now</a>
      </div>
      <div class="w-1/2 lg:w-2/12 text-center md:text-center lg:text-center lg:order-2">
        <?php
          $logo = get_field('header_logo', 'option');
          ?>
        <a href="<?php echo get_home_url(); ?>"><img class="m-auto" src="<?php echo $logo['url']; ?>" alt="<?php echo $logo['alt']; ?>"></a>          
    ...
    <nav class="flex flex-col items-center justify-center h-full text-white">
      <?php 
        foreach ($menu_items as $menu_item) {
          echo '<a class="headingThree mb-2" href="' . $menu_item->url . '">' . $menu_item->title . '</a>';
        }
      ?>
    </nav>
  </div>
